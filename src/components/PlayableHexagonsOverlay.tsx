...
interface PlayableHexagonsOverlayProps {
  centerTile: Tile;
  centerPosition: { x: number; y: number };
  onClose: () => void;
}

const PlayableHexagonsOverlay: React.FC<PlayableHexagonsOverlayProps> = ({
  centerTile,
  centerPosition,
  onClose
}) => {
  ...
  const getTilePosition = (index: number, total: number) => {
    if (total === 1) {
      return { x: centerPosition.x, y: centerPosition.y - 80 }; // Above the center tile
    }
  ...
  return (
    <div 
      className="playable-hexagons-overlay"
...
export default PlayableHexagonsOverlay;
